...
round(mean(df.cat$surveyduration) + 1.5 *sd(df.cat$surveyduration), 0) # 17 (19), Edit(Armin): 16 (18)
round(mean(df.cat$surveyduration) - 1.5 *sd(df.cat$surveyduration), 0) # 10 (8), Edit(Armin): 10 (8)

round(mean(df.spa$surveyduration) + 1.5 *sd(df.spa$surveyduration), 0) # 19 (21), Edit(Armin): 18 (21)
round(mean(df.spa$surveyduration) - 1.5 *sd(df.spa$surveyduration), 0) # 8 (6), Edit(Armin): 9 (6)
```

Sarah: The strong filter (1 SD from the mean) would exclude over half of the participants, the lax filter (1.5 SD from the mean) much less. 
Armin: Rounding in lines 148 to 152 will lead to false bounds, such that in the 1SD interval the situation is not as drastic as you described, but for the 1.5SD interval it would not change the excluded participants.

```{r exclusionTimes, results="hide"}
nrow(subjTimes.cat %>% filter(surveyduration < 10 | surveyduration > 17)) # 14 out of 25, Edit(Armin): 7/25
nrow(subjTimes.cat %>% filter(surveyduration < 8 | surveyduration > 19)) # 2 out of 25

nrow(subjTimes.spa %>% filter(surveyduration < 8 | surveyduration > 19)) # 5 out of 26 # corrected typo: 9 -> 19, Edit(Armin): 10/25 
